...
import { LogOut, Package, Navigation, X, ChevronLeft } from 'lucide-react';
...
const menuGroups = [
    {
        ...
        items: [
            { href: '/postal-client', label: 'My Shipments', icon: Package },
        ],
    }
];

interface PostalClientSidebarProps {
    isOpen: boolean;
...
export function PostalClientSidebar({ isOpen, setIsOpen, isCollapsed, setIsCollapsed }: PostalClientSidebarProps) {
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {!isCollapsed && (
                                <div className="animate-in fade-in duration-300">
                                    <h1 className="text-lg font-bold text-slate-900 leading-tight">POSTAL CLIENT</h1>
                                    <p className="text-[10px] text-slate-500 font-medium">Remote Dispatch</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className={`flex items-center gap-3 py-3 rounded-2xl border border-slate-100 bg-slate-50/50 ${isCollapsed ? 'px-0 justify-center flex-col' : 'px-3'}`}>
                        ...
                        {!isCollapsed && (
                            <div className="min-w-0 flex-1">
                                <p className="text-sm font-bold text-slate-900 truncate">{session?.user?.name || 'Local Client'}</p>
                                <p className="text-[11px] text-slate-500 truncate font-medium">{session?.user?.email || 'client@mail.com'}</p>
                            </div>
                        )}
                        <button
